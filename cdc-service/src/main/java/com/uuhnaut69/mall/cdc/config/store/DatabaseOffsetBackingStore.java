package com.uuhnaut69.mall.cdc.config.store;

import com.uuhnaut69.mall.cdc.config.CustomEmbeddedEngine;
import com.uuhnaut69.mall.cdc.config.offset.OffsetPosition;
...
public class DatabaseOffsetBackingStore extends MemoryOffsetBackingStore {
    ...
    public synchronized void start() {
        super.start();
        log.info("Starting DatabaseOffsetBackingStore");
        load();
    }
    ...
    public synchronized void stop() {
        super.stop();
        log.info("Stopped DatabaseOffsetBackingStore");
    }
    ...
    private void load() {
        ...
        if (!checkStringIsNull(fromCheckpointTime)) {
            ...
            log.info("Loaded Offset In Manual Mode");
        } else {
            offsetPositions = findOffsetInManualMode(engineName);
            log.info("Loaded Offset In Checkpoint Mode");
        }
